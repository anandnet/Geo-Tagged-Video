...
void write_metadata(String path, String fileName, String tmpVidPath,
    String tmpVidName, String data,
  ...
  _flutterFFmpeg
      .execute("-i " +
          path +
          " -movflags use_metadata_tags -metadata geo_location=\'" +
          data +
          "\' -y -r 1 -acodec copy -vcodec copy " +
          tmpVidPath)
      .then((rc) {
...
Future<Map> extract_metadata(
    String videoPath, String tmpDir, String fileName, String tagName) async {
  bool isExist = FileSystemEntity.typeSync(tmpDir + "/$fileName.txt") !=
  ...
  await _flutterFFmpeg
      .execute(
          "-i " + videoPath + " -f ffmetadata " + tmpDir + "/$fileName.txt")
      .then((rc) {
    print("FFmpeg process exited with rc $rc");
    ...
    if (l.contains(tagName)) {
      if (tagName == "geo_location") {
        ...
        try {
          ...
          print("Location data not encoded");
        }
